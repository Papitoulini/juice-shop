...
import * as imageCaptcha from './routes/imageCaptcha';
...
const collectDurationPromise = (name: string, func: (...args: any[]) => Promise<any>) => {
  return async (...args: any[]) => {
    const end = startupGauge.startTimer({
      task: name
    });
    try {
...
restoreOverwrittenFilesWithOriginals().then(() => {
  ...
  app.use('/api/Quantitys/:id', security.isAccounting(), ipfilter(['123.456.789'], {
    mode: 'allow'
  }));
  ...
  app.post('/rest/2fa/verify', new RateLimit({
    windowMs: 5 * 60 * 1000,
    max: 100
  ...
  app.post('/rest/2fa/setup', new RateLimit({
    windowMs: 5 * 60 * 1000,
    max: 100
  }), security.isAuthorized(), twoFactorAuth.setup());
  /* Disable 2FA Status for the current User */
  app.post('/rest/2fa/disable', new RateLimit({
    windowMs: 5 * 60 * 1000,
    max: 100
  }), security.isAuthorized(), twoFactorAuth.disable());
    ...
    if (name === 'SecurityQuestion') {
      {
        resource.list.fetch.after((req: Request, res: Response, context: {
          ...
          for (let i = 0; i < context.instance.length; i++) {
            if (typeof context.instance[i] === 'string') {
              context.instance[i] = {
                question: req.__(context.instance[i])
              };
            } else {
...
/* Serve metrics */
// vuln-code-snippet start exposedMetricsChallenge
/* Serve metrics */let metricsUpdateLoop: NodeJS.Timer;
