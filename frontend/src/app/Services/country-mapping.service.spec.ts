...
describe('CountryMappingService', () => {
  ...
  it('should get the country mapping directly through the rest API', inject([CountryMappingService, HttpTestingController], fakeAsync((service: CountryMappingService, httpMock: HttpTestingController) => {
    {
      let res: any;
      service.getCountryMapping().subscribe(data => res = data.map((item: any) => JSON.parse(item)));
