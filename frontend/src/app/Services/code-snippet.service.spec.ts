...
describe('CodeSnippetService', () => {
  ...
  it('should get single snippet directly from the rest api', inject([CodeSnippetService, HttpTestingController], fakeAsync((service: CodeSnippetService, httpMock: HttpTestingController) => {
    {
      let res: {
        snippet: string;
      };
  ...
  it('should get list of challenges with code snippets directly from the rest api', inject([CodeSnippetService, HttpTestingController], fakeAsync((service: CodeSnippetService, httpMock: HttpTestingController) => {
    {
      let res: Array<string>;
      service.challenges().subscribe(data => res = data);
      const req = httpMock.expectOne('http://localhost:3000/snippets');
      req.flush({
        challenges: ['directoryListingChallenge', 'accessLogDisclosureChallenge', '...', 'xssBonusChallenge']
      });
      tick();
      expect(req.request.method).toBe('GET');
