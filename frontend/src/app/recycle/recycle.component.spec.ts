...
describe('RecycleComponent', () => {
  ...
  beforeEach(waitForAsync(() => {
    ...
    TestBed.configureTestingModule({
      imports: [RouterTestingModule, TranslateModule.forRoot(), HttpClientTestingModule, BrowserAnimationsModule, ReactiveFormsModule, MatFormFieldModule, MatInputModule, MatButtonModule, MatCardModule, MatCheckboxModule, MatDatepickerModule, MatNativeDateModule, MatIconModule, MatToolbarModule, MatTableModule, MatRadioModule, MatTooltipModule, MatDialogModule, MatDividerModule],
      declarations: [RecycleComponent, AddressComponent],
      providers: [{
        provide: RecycleService,
        useValue: recycleService
      }, {
        provide: UserService,
  ...
  it('should display pickup message and reset recycle form on saving when pickup is selected', () => {
    ...
    expect(component.initRecycle).toHaveBeenCalled();
    expect(component.addressComponent.load).toHaveBeenCalled();
    expect(translateService.get).toHaveBeenCalledWith('CONFIRM_RECYCLING_PICKUP', {
      pickupdate: '10/7/2018'
    });
  });
