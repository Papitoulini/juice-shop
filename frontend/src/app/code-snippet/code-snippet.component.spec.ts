...
describe('CodeSnippetComponent', () => {
  ...
  beforeEach(waitForAsync(() => {
    ...
    TestBed.configureTestingModule({
      imports: [CookieModule.forRoot(), TranslateModule.forRoot(), HttpClientTestingModule, MatDividerModule, MatDialogModule],
      ...
      providers: [{
        ...
        useValue: codeSnippetService
      }, {
        provide: CodeFixesService,
        useValue: codeFixesService
      }, {
        provide: VulnLinesService,
        useValue: vulnLinesService
      }, {
  ...
  it('should set the retrieved snippet', () => {
    ...
    component.ngOnInit();
    expect(component.snippet).toEqual({
      snippet: 'Snippet'
    });
  });
  ...
  it('should set an error on snippet retrieval as the snippet', () => {
    codeSnippetService.get.and.returnValue(throwError({
      error: 'Error'
    }));
    component.ngOnInit();
    expect(component.snippet).toEqual({
  ...
  it('correctly submitted vulnerable lines toggle positive verdict for "Find It" phase', () => {
    ...
    vulnLinesService.check.and.returnValue(of({
      verdict: true
    }));
  ...
  it('correctly submitted fix toggles positive verdict for "Fix It" phase', () => {
    ...
    codeFixesService.check.and.returnValue(of({
      verdict: true
    }));
    component.checkFix();
  ...
  it('should remember the original order of available code fix options when shuffling', () => {
    ...
    component.shuffle();
    expect(component.randomFixes).toContain({
      fix: 'Fix 1',
      index: 0
    });
    expect(component.randomFixes).toContain({
