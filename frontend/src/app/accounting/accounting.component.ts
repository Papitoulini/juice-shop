...
export class AccountingComponent implements AfterViewInit, OnDestroy {
  ...
  public tableData: { id: number, name: string, price: number }[]
  public dataSource
  ...
  loadProducts () {
    this.productSubscription = this.productService.search('').subscribe((tableData: { id: number, name: string, price: number }[]) => {
      this.tableData = tableData
      this.dataSource = new MatTableDataSource<{ id: number, name: string, price: number }>(this.tableData)
