...
describe('OAuthComponent', () => {
  ...
  beforeEach(waitForAsync(() => {
    ...
    userService.oauthLogin.and.returnValue(of({
      email: ''
    }));
    ...
    TestBed.configureTestingModule({
      ...
      }]), ReactiveFormsModule, CookieModule.forRoot(), TranslateModule.forRoot(), MatInputModule, MatIconModule, MatCardModule, MatFormFieldModule, MatCheckboxModule, HttpClientTestingModule, MatTooltipModule],
      providers: [{
        provide: ActivatedRoute,
        useValue: {
          snapshot: {
            data: {
              params: '?alt=json&access_token=TEST'
            }
          }
        }
      }, {
  ...
  it('removes authentication token and basket id on failed OAuth login attempt', fakeAsync(() => {
    userService.oauthLogin.and.returnValue(throwError({
      error: 'Error'
    }));
    component.ngOnInit();
  ...
  it('logs in user even after failed account creation as account might already have existed from previous OAuth login', fakeAsync(() => {
    ...
    userService.save.and.returnValue(throwError({
