...
describe('DataExportComponent', () => {
  ...
  beforeEach(waitForAsync(() => {
    ...
    TestBed.configureTestingModule({
      ...
      imports: [HttpClientTestingModule, TranslateModule.forRoot(), MatFormFieldModule, ReactiveFormsModule, BrowserAnimationsModule, MatInputModule, MatCardModule, MatRadioModule, MatButtonModule],
      providers: [{
        provide: ImageCaptchaService,
        useValue: imageCaptchaService
      }, {
        provide: DataSubjectService,
        useValue: dataSubjectService
      }, TranslateService]
    }).compileComponents();
  }));
  beforeEach(() => {
  ...
  it('should show the confirmation and fetch user data and reset data export form on requesting data export', () => {
    dataSubjectService.dataExport.and.returnValue(of({
      confirmation: 'Data being exported',
      userData: '{ user data }'
    }));
  ...
  it('should clear the form and display error if exporting data fails', fakeAsync(() => {
    dataSubjectService.dataExport.and.returnValue(throwError({
      error: 'Error'
    }));
