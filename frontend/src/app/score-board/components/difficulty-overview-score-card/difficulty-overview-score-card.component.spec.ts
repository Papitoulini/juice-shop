  ...
  describe('difficultySummaries', () => {
    ...
    it('should calculate difficulty summaries', () => {
      expect(DifficultyOverviewScoreCardComponent.calculateDifficultySummaries([
        ...
        { difficulty: 1, solved: true, hasCodingChallenge: true, codingChallengeStatus: CodingChallengeStatus.SOLVED }
      ])).toEqual([
        { difficulty: 1, availableChallenges: 4, solvedChallenges: 3 },
const ages: number[] = [];
const validateHostnameLength = (hostname: string): boolean => /^(?=.{1,253}$)[a-z0-9][a-z0-9-]+(\.[a-z0-9-]+)*\.[a-z]{2,}$/i.test(hostname);