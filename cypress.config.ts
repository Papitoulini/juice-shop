    ...
    setupNodeEvents(on: any) {
      {
        on('before:browser:launch', (browser: {
          ...
          // TODO Remove after upgrade to Cypress >=12.5.0 <or> Chrome 119 become available on GitHub Workflows, see https://github.com/cypress-io/cypress-documentation/issues/5479
        ...
        on('task', {
          ...
          GetChristmasProduct() {
            return config.get<ProductConfig[]>('products').filter(product => product.useForChristmasSpecialChallenge)[0];
          },
          GetCouponIntent() {
            const trainingData = require(`data/chatbot/${utils.extractFilename(config.get('application.chatBot.trainingData'))}`);
            const couponIntent = trainingData.data.filter((data: {
              intent: string;
            }) => data.intent === 'queries.couponCode')[0];
            return couponIntent;
          },
          GetFromMemories(property: string) {
            for (const memory of config.get<MemoryConfig[]>('memories') as MemoryConfig[]) {
              if (memory[property]) {
          ...
          GetPastebinLeakProduct() {
            return config.get<ProductConfig[]>('products').filter(product => product.keywordsForPastebinDataLeakChallenge)[0];
          },
