...
exports.accessControlChallenges = () => ({
  ...
  challengeUtils.solveIf(challenges.missingEncodingChallenge, () => {
    return utils.endsWith(url.toLowerCase(), '%f0%9f%98%bc-%23zatschi-%23whoneedsfourlegs-1572600969477.jpg');
  });
...
exports.errorHandlingChallenge = () => (err: unknown, req: Request, {
  statusCode
  ...
  if (token) {
    {
      ...
      jwt.verify(token, security.publicKey, (err: VerifyErrors | null, verified: object) => {
        if (err === null) {
...
function dangerousIngredients() {
  return config.get<ProductConfig[]>('products').flatMap(product => product.keywordsForPastebinDataLeakChallenge).filter(Boolean).map(keyword => {
    return {
      [Op.like]: `%${keyword}%`
    };
  });
