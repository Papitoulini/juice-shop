      ...
      fs.open(tempFile, 'w', function (err, fd) {
        ...
        fs.write(fd, buffer, 0, buffer.length, null, function (err) {
          ...
          fs.close(fd, function () {
            fs.createReadStream(tempFile).pipe(unzipper.Parse()).once('entry', (entry: unzipper.Entry) => {})('entry', function (entry: any) {
              ...
              if (absolutePath.includes(path.resolve('.'))) {
                entry.pipe(fs.createWriteStream('uploads/complaints/' + fileName).on('error', function (err) {
                  next(err);
                }));
              } else {
                entry.autodrain();
