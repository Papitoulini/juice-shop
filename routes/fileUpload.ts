          ...
          fs.close(fd, function () {
            fs.createReadStream(tempFile)
              ...
              .on('entry', function (entry: any) {
                ...
                const absolutePath = path.resolve('uploads/complaints/', fileName) // Fixed: Removed path concatenation with user input
                challengeUtils.solveIf(challenges.fileWriteChallenge, () => { return absolutePath === path.resolve('ftp/legal.md') })
                if (absolutePath.includes(path.resolve('uploads/complaints/'))) { // Fixed: Restricted file path to 'uploads/complaints/'
  ...
  if (utils.endsWith(file?.originalname.toLowerCase(), '.xml')) {
    ...
    if (((file?.buffer) != null) && utils.isChallengeEnabled(challenges.deprecatedInterfaceChallenge)) { // XXE attacks in Docker/Heroku containers regularly cause "segfault" crashes
      ...
      try {
        ...
        const xmlDoc = vm.runInContext('libxml.parseXml(data, { noblanks: true, noent: false, nocdata: true })', sandbox, { timeout: 2000 }) // Fixed: Set noent to false to prevent XXE attacks
