...
router.post('/', async (req: Request<Record<string, unknown>, Record<string, unknown>, DataErasureRequestParams>, res: Response, next: NextFunction): Promise<void> => {
  ...
  try {
    ...
    if (req.body.layout) {
      const sanitizedLayout = req.body.layout.replace(/\.\./g, '') // Sanitize user input to prevent path traversal
      const filePath: string = path.resolve(__dirname, '..', 'views', sanitizedLayout).toLowerCase()
