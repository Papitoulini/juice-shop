...
const lang: { key: string, lang: string, icons?: string[] } = {
  key,
  ...
  icons: locale?.icons,
    ...
    async function calcPercentage(fileContent: object, enContent: object): Promise<number> {
      const totalStrings = Object.keys(enContent).length as number;
