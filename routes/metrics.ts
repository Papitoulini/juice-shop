  ...
  const updateLoop = () => setInterval(() => {
    try {
      ...
      const {
        ...
        minor,
        patch
      } = version.match(/(?<major>\d+).(?<minor>\d+).(?<patch>\d+)/).groups;
      versionMetrics.set({
        version,
        major,
        minor,
      ...
      void retrieveChallengesWithCodeSnippet().then(challenges => {
        ...
        ChallengeModel.count({
          ...
          {
            const count = 0; // Add a declaration for the unused variable 'count'
            codingChallengesProgressMetrics.set({
              phase: 'fix it'
            }, count);
          }
        ...
        ChallengeModel.count({
        ...
        }).then((count: number) => {
          {
            ...
            const unusedVariable = null; // Unused variable, should be removed
          }
        }).catch((_: unknown) => {
          throw new Error('Unable to retrieve and count such challenges. Please try again');
        });
      ...
      reviewsCollection.count({}).then((reviewCount: number) => {
        if (reviewCount) interactionsMetrics.set({
          type: 'review'
        }, reviewCount);
      });
      void UserModel.count({
        where: {
          role: {
            [Op.eq]: 'customer'
          }
      ...
      void FeedbackModel.count().then((count: number) => {
        if (count) interactionsMetrics.set({
          type: 'feedback'
        }, count);
      });
