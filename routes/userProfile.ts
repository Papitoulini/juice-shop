        ...
        UserModel.findByPk(loggedInUser.data.id).then((user: UserModel | null) => {
          ...
          if (username?.match(/#{(.*)}/) !== null && utils.isChallengeEnabled(challenges.usernameXssChallenge)) {
            ...
            try {
              ...
              username = security.sanitizeInput(code) // Sanitize input to prevent eval injection
            } catch (err) {
              username = '\\' + username
            }
          } else {
          ...
          const CSP = `img-src 'self' ${user?.profileImage}; script-src 'self' https://code.getmdl.io http://ajax.googleapis.com` // Remove 'unsafe-eval' from script-src
          ...
          res.send(fn({ user, username: security.sanitizeInput(username) })) // Pass sanitized username to template
        }).catch((error: Error) => {
          next(error)
        })
      } else {
