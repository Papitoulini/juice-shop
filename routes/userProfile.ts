...
throw new Error('Username is null')
              ...
              username = decodeURIComponent(username) // Safe alternative to eval
            } catch (err) {
              username = '\\' + username
            }
          } else {
...
template = template.replace(/_primDark_/g, theme.primDark)
          ...
          const CSP = `img-src 'self' ${user?.profileImage ? user.profileImage : ''}; script-src 'self' 'unsafe-eval' https://code.getmdl.io http://ajax.googleapis.com`
