        ...
        UserModel.findByPk(loggedInUser.data.id).then((user: UserModel | null) => {
          ...
          if (username?.match(/#{(.*)}/) !== null && utils.isChallengeEnabled(challenges.usernameXssChallenge)) {
            ...
            try {
              ...
              username = security.sanitizeSecure(code) // Sanitize user input to prevent code injection
            } catch (err) {
              username = '\\' + username
            }
          } else {
          ...
          const CSP = `img-src 'self' ${user?.profileImage}; script-src 'self' https://code.getmdl.io http://ajax.googleapis.com` // Remove 'unsafe-eval' from CSP
