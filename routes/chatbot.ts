res.status(200).json({
        ...
        body: bot.training.state ? bot.greet(DOMPurify.sanitize(`${user.id}`)) : DOMPurify.sanitize(`${config.get<string>('application.chatBot.name')} isn't ready at the moment, please wait while I set things up`)
      })
    } catch (err) {
