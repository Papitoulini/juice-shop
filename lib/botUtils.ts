...
export function couponCode (query: string) {
  const solveResult = challengeUtils.solveIf(challenges.bullyChatbotChallenge, () => { return true })
  return {
    action: 'response',
...
export function testFunction (query: string): { action: string, body: string } {
  return {
    action: 'response',
