...
const files = await fs.readdir(currPath)
      const moreMatches = await findFilesWithCodeChallenges(
        files.map(file => path.posix.join(currPath, path.posix.basename(file)))
      )
...
if (new RegExp(`vuln-code-snippet vuln-line.*${sanitizeRegexInput(challengeKey)}`, 'u').exec(lines[i]) != null) {
      vulnLines.push(i + 1)
    } else if (new RegExp(`vuln-code-snippet neutral-line.*${sanitizeRegexInput(challengeKey)}`, 'u').exec(lines[i]) != null) {
