...
if (RegExp(`vuln-code-snippet vuln-line.*${challengeKey.replace(/[()?[\]\\]/g, '\\$&')}`).test(lines[i])) {
      vulnLines.push(i + 1)
    } else if (RegExp(`vuln-code-snippet neutral-line.*${challengeKey.replace(/[()?[\]\\]/g, '\\$&')}`).test(lines[i])) {
