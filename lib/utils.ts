...
export const queryResultToJson = <T,>(data: T, status: string = 'success'): {
  data: T;
  status: string;
} => {
  return {
...
export function getChallengeEnablementStatus(challenge: Challenge, safetyModeSetting: SafetyModeSetting = config.get<SafetyModeSetting>('challenges.safetyMode'), isEnvironmentFunctions: {
...
export function isChallengeEnabled(challenge: Challenge): boolean {
  const {
    enabled
  } = getChallengeEnablementStatus(challenge);
  return enabled;
}
