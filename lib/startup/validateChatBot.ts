...
export default function validateChatBot (trainingData: { [key: string]: any }, exitOnFailure = true) {
  let success = true
...
export const checkIntentWithFunctionHandlerExists = (trainingData: { [key: string]: any }, intent: string, handler: string) => {
  let success = true
  const intentData = trainingData.data.filter((data: { [key: string]: any }) => data.intent === intent)
