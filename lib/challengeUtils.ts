...
export const solve = function (challenge: {
  solved: boolean;
  save(): Promise<{
    difficulty: number;
    key: string;
...
export const sendNotification = function (challenge: {
  difficulty?: number;
  key: string;
  name: string;
  description?: string;
  ...
  if (!notSolved(challenge)) {
    ...
    const notification = {
      key: challenge.key,
      name: challenge.name,
      challenge: challenge.name + ' (' + entities.decode(sanitizeHtml(challenge.description, {
        allowedTags: [],
        allowedAttributes: {}
      })) + ')',
      flag,
...
export const notSolved = (challenge: {
  solved?: boolean;
