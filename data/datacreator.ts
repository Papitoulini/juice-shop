...
export default async () => {
  const creators = [createSecurityQuestions, createUsers, createChallenges, createRandomFakeUsers, createProducts, createBaskets, createBasketItems, createAnonymousFeedback, createComplaints, createRecycleItem, createOrders, createQuantity, createWallet, createDeliveryMethods, createMemories, prepareFilesystem];
...
async function createChallenges() {
  ...
  await Promise.all(challenges.map(async ({
    name,
    category,
    description,
    difficulty,
...
async function createWallet() {
  const users = await loadStaticUserData();
  return await Promise.all(users.map(async (user: StaticUser, index: number) => {
...
async function createDeliveryMethods() {
  ...
  await Promise.all(deliveries.map(async ({
    name,
    price,
    deluxePrice,
    eta,
...
async function createRandomFakeUsers() {
  ...
  return await Promise.all(new Array(config.get('application.numberOfRandomFakeUsers')).fill(0).map(async () => await UserModel.create({
    email: getGeneratedRandomFakeUserEmail(),
    password: makeRandomString(5)
  })));
}
async function createQuantity() {
  return await Promise.all(config.get<ProductConfig[]>('products').map(async (product, index) => {
...
async function createMemories() {
  const memories = [MemoryModel.create({
...
async function createProducts() {
  ...
  return await Promise.all(products.map(async ({
    reviews = [],
...
async function createSecurityQuestions() {
  ...
  await Promise.all(questions.map(async ({
    question
  }) => {
    try {
